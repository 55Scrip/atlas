"""FastAPI DI wiring for Decision Path & Required Progress -- reuses
every existing provider unchanged, the same "compose, never duplicate"
pattern `atlas.alpha.recommendation_conviction.api.dependencies`
already established."""
...
__all__ = ["get_decision_path_result_repository", "get_decision_path_service"]


def get_decision_path_result_repository(
    engine: Engine = Depends(get_decision_engine),
) -> SqlAlchemyDecisionPathResultRepository:
    create_decision_path_result_table(engine)
    return SqlAlchemyDecisionPathResultRepository(engine)


def get_decision_path_service(
    ...
    result_repository: SqlAlchemyDecisionPathResultRepository = Depends(get_decision_path_result_repository),

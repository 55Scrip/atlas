...
**Refreshed:** 2026-08-03, closing the Investment Case, Portfolio, Watchlist, Daily Brief, and Discover findings named in §4 and §6 below, per the completed Product Architecture Reconciliation and APS Expansion. This refresh updates specific findings only; it is not a new baseline and does not supersede this document, per §8's own supersession discipline — the document's own Created date, status, and governing scope remain unchanged.
...
| **Product Architecture** | `docs/atlas_product_architecture/APP-000`, `APP-001` (v0.4), `APS-001–009` | Translates settled Core concepts into named Product Concepts a Product author can specify features against, and now includes Investment Case, Portfolio, Watchlist, Daily Brief, and Discover as the single integrated lineage (`Architecture-Governance.md` §8.7.4). Currently entirely Draft. |
...
- **Investment Case** — intended for Alpha, and **now a settled Product Architecture concept** as of `APP-001` v0.4 §3.13: the confirmed, 1:1 product-facing name for Core's own `Case`, distinct from, and enclosing, Decision Context. This closes the Product-layer half of the "future Investment Case / Portfolio Product Architecture treatment" `UX-012` itself named as a precondition — see `docs/Architecture-Governance.md` §8.7.4. `UX-012`'s own text has not itself been updated to reference this resolution; a future UX-track update remains required before Alpha implementation may cite `UX-012` as fully reconciled on this term. Portfolio, Watchlist, Daily Brief, and Discover are likewise now specified, at Product Architecture Draft status, in `APS-006` through `APS-009`.
...
- **Draft documentation:** all of Product Architecture (`APP-000`, `APP-001` v0.4, `APS-001–009`, now the single integrated lineage per `Architecture-Governance.md` §8.7.4), and the active UX Design System / Component Specification corpus below the Doctrine — see §8.2.

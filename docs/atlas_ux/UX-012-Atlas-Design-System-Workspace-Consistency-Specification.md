...
This is the single authoritative reference for every future Atlas interface, component, interaction, visual pattern, and governance decision. Every future Workspace, component, interaction, and visual pattern derives from this specification. Prior part documents (012A–D) are superseded by this assembled version for the content this document itself specifies; where a Part document's own content has since been independently, subsequently governed beyond what this document captures, that Part document remains the current authority — see the Correction Notice below for Design Tokens specifically.
...
**Correction Notice (Atlas UX Architecture Governance Authority Resolution task — 2026-08-02):** This is a later, additive correction; it does not revise, replace, or reopen any notice above, each of which remains historically accurate for what it addressed. This document's own introduction (above) stated: *"All prior part documents (012A–D) are superseded by this assembled version."* The completed Cross-Document Token Consistency Audit (2026-08-02) found this blanket claim in direct tension with the completed Reasoning Token Architecture program (Phases 1 through 3C, 2026-08-02), which consistently and correctly treated `UX-012D-Atlas-Design-System-Governance-Tokens-Evolution.md` as the sole canonical Design Token authority. Independent re-investigation, conducted for this correction, confirms the audit's finding, identifies its precise scope, and resolves it narrowly.

Prior text is preserved verbatim, in quotation, above; it is not erased. This correction narrows it, for Design Tokens specifically: **`UX-012D` remains the current, authoritative, continuously-governed source for Atlas's Design Token philosophy, taxonomy, naming conventions, and governance model. This document's own Sections 54–56 ("Part VI: Design Tokens") are a snapshot of that model as it existed at this document's own original assembly — not a supersession of `UX-012D`'s own subsequent, independently governed evolution.** Three findings support this scoping. First, `UX-012A`'s own Status line already establishes the narrow-supersession pattern this correction now applies consistently: it records itself "Superseded" only for the one specific item (§5 hierarchy wording) this document actually corrected, explicitly preserving the remainder of its own content as not superseded — this document's own blanket phrasing at the introduction was never reconciled against that same discipline. Second, neither `UX-012B` nor `UX-012C` records itself as superseded at all (their own Status lines read "Component Specification Complete" and "Interaction Specification Complete"), and `UX-012D`'s own Status line reads "Governance Specification Complete" — none of the four Part documents' own self-declarations support a blanket total supersession; only this document's own introduction asserted one. Third, and decisive for Design Tokens specifically: `UX-012D` has since undergone six dedicated, independently governed correction phases (Reasoning Token Architecture Phases 1 through 3C, 2026-08-02) that materially extended its own token model — a Text Hierarchy, a Surface Hierarchy, a Motion Architecture, an Interaction group, a canonical Attribution & Action Text mapping, and a closed top-level-category rule, among other additions — none of which is, or could be, reflected in this document's own Sections 54–56. This document's own worked-example tokens at those sections (`conclusion.text`, `conclusion.text.primary`, `contradiction.border.blocking`) accordingly predate, and do not match, `UX-012D`'s own current canonical forms (`color.text.primary`/`.secondary`/`.tertiary`; `color.border.contradiction.informational`/`.material`/`.unresolved`) — direct, checkable evidence that this document's own Design Token content is a historical snapshot, not a current, superseding specification. `ADR-001-Missing-Source-Volume-Governance.md`'s own governing principle applies directly here: *"Partially corroborated claims are distinguished from wholly unsupported claims. Where a broad concept is independently attested elsewhere... that distinction is preserved and stated, rather than collapsed into one blanket status."* This document's own broad Design Token concept (Sections 54–56) remains a legitimate, corroborated historical account of `UX-012D`'s own earlier state; its specific, current, literal token forms are not current, and this correction states that distinction rather than collapsing it.

This correction changes no token, component, interaction, or accessibility behavior anywhere in this document or any document it governs. It does not reopen, narrow, or extend this document's own authority over Foundation (`UX-012A`), Component (`UX-012B`), or Interaction (`UX-012C`) content, which remains exactly as this document and those documents' own existing Status lines already establish — this correction is scoped to Design Tokens only, per the governing task's own explicit scope. Sections 54–56 themselves are not rewritten, corrected, or normalized by this notice — they remain exactly as previously written, now accompanied by this notice's own scoping clarification, consistent with this corpus's own non-erasure, additive-correction practice. `UX-012D` is not modified by this correction — no documentary evidence found during this task's own investigation required a change to that document; the conflict originated, and is resolved, entirely within this document's own introduction.

**Correction Notice (Atlas UX Architecture Token Architecture Release Polish (Final Sprint) task — 2026-08-02):** This is a later, additive correction; it does not revise, replace, or reopen either notice above, each of which remains historically accurate for what it addressed at the time. The Governance Authority Resolution notice above states that "Sections 54–56 themselves are not rewritten, corrected, or normalized by this notice" — that remains true of that notice specifically; this later, separate, explicitly-authorized task performs exactly that normalization, for illustrative examples only. Three obsolete worked-example tokens, cited by the Governance Authority Resolution notice above as evidence of staleness, are corrected below to their current canonical `UX-012D` §3 forms: `conclusion.text` (Canonical Glossary, "Semantic Token" entry, and §54) is corrected to `color.text.primary`; `conclusion.text.primary` (§56, "Tokens" naming example) is corrected to `color.text.primary`; `contradiction.border.blocking` (same §56 example) is corrected to `color.border.contradiction.unresolved` — the actual canonical token for the Blocking Challenge severity tier the original example illustrated, per `UX-012D` §3's own Contradiction group and the already-resolved Blocking/Unresolved label-token split. `motion.expand.duration`, the third token in the same §56 example, was already canonical and is unchanged. **These are illustrative-example corrections only.** No surrounding explanatory text is altered, no document structure changes, and Sections 54–56 remain, exactly as the notice above already established, a historical snapshot of `UX-012D`'s own earlier state — this correction only updates the specific token spellings those examples use, so they no longer contradict the canonical forms `UX-012D` §3 currently defines. No new token, no architecture change, and no change to this document's own scope or authority anywhere else.
...
**Semantic Token** — A token named for its meaning rather than its visual value (e.g., `color.text.primary` not `off-white`).
...
Semantic tokens decouple meaning from implementation. When the visual implementation changes (a color is adjusted, a typeface is updated), the semantic token name remains stable. Code that references `color.text.primary` continues to express the right meaning even if the specific color value changes.
...
**Tokens:** `category.role.variant`. Examples: `color.text.primary`, `color.border.contradiction.unresolved`, `motion.expand.duration`.

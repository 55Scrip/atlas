...
**Correction Notice (Atlas UX Architecture Status Badge Token Architecture Correction task — 2026-08-02):** This is a later, additive correction; it does not revise, replace, or reopen either notice above. `UX-013A-Atlas-Component-Specification-Foundation-Components.md`'s own Status Badge component (§13) previously referenced an unsupported `status.*` token namespace (e.g. `status.draft.text`, `status.historical.background`, `status.warning.text`) that never existed anywhere in this document, confirmed by the completed Status Badge Token Family Architecture Investigation (2026-08-02). That investigation found that existing token groups in Section 3, below, already cover most Status Badge variants — Historical Content already governs the Historical badge type; the Warning/Contradiction mapping already governs the Warning badge type; the existing `color.semantic.amber` value already governs Monitoring: Approaching; and Draft, Saved, Monitoring: Active, and Updated require no distinct semantic color at all, per `UX-012B` §13's own literal state descriptions. **No general `status.*` token family is adopted here or anywhere in this document.** Two bounded semantic gaps were found to have no existing exact equivalent and are closed by the two small additions below, each added to its own already-established semantic group rather than as a new category: a canonical red semantic accent, for Monitoring: Triggered (added to the existing Monitoring group); and a badge-level text token, for the Completed/Recorded Status Badge (added to the existing Completed group). No other Status Badge variant receives a new token here. This notice does not claim `status.*` tokens ever existed as valid entries in this document — they did not; the correction is confined to `UX-013A`'s own references, addressed in that document directly.

**Correction Notice (Atlas UX Architecture Reasoning Token Architecture Phase 1: UX-012D Foundations task — 2026-08-02):** This is a later, additive correction; it does not revise, replace, or reopen any notice above. The completed Full Reasoning-Tier Token Architecture Audit (2026-08-02) found that `UX-013B-Atlas-Component-Specification-Reasoning-Components.md` used unsupported token namespaces (`text.*`, `border.*`, `reasoning.*`, `conclusion.*`, `status.assumption.*`, `status.scenario.*`, `focus.ring.color`, `editing.field.background`) in part because this document itself only conceptually described several of the underlying visual concepts those namespaces were reaching for — a neutral text-color hierarchy, a neutral surface hierarchy, a green semantic accent, and an editing-presentation opacity value — without ever giving any of them a literal token. This correction formalizes only those smallest shared foundations, each placed under its own already-established category or semantic group, never as a new top-level category: `color.text.primary` (normalized, not duplicated — it already existed as a naming-convention example; it now also has a full semantic-group definition), `color.text.secondary`, `color.text.tertiary` (Section 3, new Text Hierarchy group); `surface.background`, `surface.primary`, `surface.elevated`, `surface.panel` (Section 3, new Surface Hierarchy group); `color.semantic.green` (Section 3, Monitoring-adjacent accent set, alongside amber and red); one internal-consistency fix resolving this document's own conflicting focus-ring naming order between Section 3 and Section 15 (canonical form: `color.focus.ring`, `width.focus.ring`, `radius.focus.ring`); and `opacity.editing.overlay` (Section 2/3, formalizing the already-stated 0.05–0.08 editing-overlay range). **No `reasoning.*`, `conclusion.*`, `status.*`, bare `text.*`, bare `border.*`, or `editing.*` top-level category is adopted here or anywhere in this document.** No Challenge- or Assumption-specific surface token is added — Part 7 of the governing task found no forcing function requiring one; existing border/accent color tokens, composed with the general `opacity.*` category, already cover the need (see Section 3, Contradiction/Monitoring groups). This correction changes no Product semantic, component architecture, ownership, lifecycle, interaction, or accessibility behavior anywhere in this document or any document it governs. It does not remap `UX-013B`'s own references — that remains a separate, later task, for which these additions are only the prerequisite.

**Correction Notice (Atlas UX Architecture Reasoning Token Architecture Phase 3A: Canonical Reasoning Token Mapping task — 2026-08-02):** This is a later, additive correction; it does not revise, replace, or reopen any notice above. `UX-013B` previously used unsupported `reasoning.*` and `conclusion.*` top-level token namespaces for thirteen distinct references (Conclusion Statement/Sub-statement, Supporting Factor/Challenge/Assumption/Scenario/Reasoning Block names, Opportunity Summary and Recommendation statements, and four Opportunity Cost sub-fields). **Direct inspection of every occurrence found each one to be a text-color role, not a typography role** — confirmed by each occurrence's own "Visual Property" label (the great majority explicitly named "...color") and by cross-verification against already-corrected sibling rows in the identical tables (e.g., "What it offered," "Why not chosen," and "Primary reason," all already mapped to the Text Hierarchy this document's own Phase 1 correction established). Typography for these same Reasoning components is unaffected and was never in question — it is already fully and separately governed by `UX-013B`'s own generic `type.role1.*` through `type.role5.*` system, confirmed present and correct in that document's own Typography Tokens table. **This finding means no new token category, and no new token of any kind, is required in this document for Phase 3A.** Every one of the thirteen unsupported references reuses the already-existing `color.text.primary` or `color.text.secondary` Text Hierarchy tokens (Section 3, Phase 1 correction, above) — the identical tokens this document's own Phase 1 task already formalized for exactly this purpose. **No `reasoning.*` or `conclusion.*` top-level category is adopted here or anywhere in this document.** This correction changes no Product semantic, component architecture, ownership, lifecycle, interaction, or accessibility behavior. Motion, Interaction, attribution-text, and action-text token families are unaffected and remain separate, later work. The full per-token mapping and its own rationale are documented in the Canonical Reasoning Token Mapping subsection, Section 3, below; the corresponding `UX-013B` remap is performed in that document directly.

**Correction Notice (Atlas UX Architecture Reasoning Token Architecture Phase 3B: Motion & Interaction Foundations task — 2026-08-02):** This is a later, additive correction; it does not revise, replace, or reopen any notice above. `UX-013B-Atlas-Component-Specification-Reasoning-Components.md` referenced eight bare `motion.*` tokens (`motion.expand`, `motion.collapse`, `motion.insert`, `motion.remove`, `motion.highlight`, `motion.update`, `motion.fade`, `motion.loading`) and one `interaction.hover.background` token, none of which named a literal token this document defined. This document's own Section 2 already committed conceptually to the underlying model — twelve motion events, each governed by one of four named duration categories (immediate: 0ms, brief: 100–150ms, standard: 200–250ms, deliberate: 350–400ms) and one of three named easing function references (ease-out, ease-in, ease-in-out) — and already used `motion.expand.duration` as its own literal naming-convention worked example (Section 4). This correction instantiates that already-committed model as literal tokens, for exactly the seven motion events a consuming document currently uses (Expand, Collapse, Insert, Remove, Highlight, Update, Fade) — not all twelve; the remaining events (Open, Close, Replace, Navigate) have no current consuming reference and are not tokenized speculatively, per this document's own established practice (see `color.semantic.blue`, Section 3, left unformalized on the same basis). Each of the seven event tokens is a named alias of one shared duration-category token and one shared easing-reference token, never an independent value — preferring shared aliases over per-component unique values, consistent with this document's own global-to-semantic-to-component token tier model (Section 13). `motion.loading` is not formalized as a Motion token: direct comparison against `UX-013B`'s own already-existing Reduced Motion section (its Loading skeleton fallback: "static, no shimmer") found it fully governed by the already-existing, untouched `opacity.loading.pulse.min`/`.max` tokens (Section 3, Loading group, above) — a second Motion token for the identical effect would be Token Duplication, an anti-pattern this document's own Section 12 already names. `interaction.hover.background` was found to name two defects at once: it implies a fixed background color, where this document's own Section 2 already describes hover as "a surface lightening delta" — an adjustment applied to an existing surface, not a surface of its own; and `interaction.*` is not among this document's own closed top-level token categories (Section 4's own Governance Clarifications) — the same defect class as the already-rejected `reasoning.*`, `conclusion.*`, and `status.*` namespaces. The corrected token, `opacity.interaction.hover`, is named under the already-established `opacity.*` category, following the identical pattern already used for `opacity.editing.overlay`. No specific numeric delta is established by any governing source examined; none is invented here — the token names the role, and its own numeric value remains an implementation-tier decision. This correction adds one new Motion Architecture group and one new Interaction group to Section 3, below. It does not add a new top-level token category, does not alter Section 14's already-existing reduced-motion accessibility requirement ("All motion must reference the reduced-motion flag and produce the instantaneous variant when the flag is active"), and changes no Product semantic, component architecture, ownership, lifecycle, or accessibility behavior. The corresponding `UX-013B` remap is performed in that document directly.

**Correction Notice (Atlas UX Architecture Reasoning Token Architecture Phase 3C: Attribution & Action Text task — 2026-08-02):** This is a later, additive correction; it does not revise, replace, or reopen any notice above. This document previously deferred `text.attribution.atlas` and the `text.action.*` family (Phase 2, Phase 3A, and Phase 3B notices, above) pending dedicated investigation. Fresh reading of `UX-012A-Atlas-Design-System-Foundations.md` and `UX-012B-Atlas-Design-System-Components-Reusable-Patterns.md` found the governing evidence this correction required: `UX-012A` states that "primary action labels are in the same scale as secondary body text... the action hierarchy is communicated through placement and visual weight, not through outsized typography" (a typographic-scale rule, distinct from and not in conflict with color); `UX-012B` §15's own exhaustive Action Components taxonomy explicitly assigns each action tier its own color/weight: Primary Action — "the highest-emphasis action control... primary text color"; Secondary Action — "a link-style treatment... at secondary text weight"; Inline Action — explicitly Tertiary emphasis, with named examples including "Edit," "Dismiss," and navigation-style links ("View source →"); Section Action — also explicitly Tertiary. Every remaining `UX-013B` attribution/action reference was found, on direct inspection, to be a text-color role already governed by the existing Text Hierarchy (Section 3, Phase 1 correction) once matched against this taxonomy — none names a genuinely new visual concept. A corpus sweep conducted as part of this task's own required investigation also found two further unsupported wildcard references not on the task's own expected list — `action.primary.*` and `action.secondary.*`, both on `UX-013B`'s Recommendation component — confirming the investigation's own "do not assume the list is complete" instruction was warranted. **No new token, and no new token category, is introduced by this correction — every reference is remapped to the already-existing `color.text.primary`, `color.text.secondary`, or `color.text.tertiary` Text Hierarchy token.** This correction adds one mapping-contract subsection to Section 3, below, following the same documentary pattern the Canonical Reasoning Token Mapping (Phase 3A) already established — not a new semantic token group. This correction changes no Product semantic, component architecture, ownership, lifecycle, interaction, or accessibility behavior; `motion.*`, `interaction.*`, `reasoning.*`, `conclusion.*`, `border.*`, `surface.*`, `focus.*`, `opacity.*`, typography, and accessibility content are unaffected. The corresponding `UX-013B` remap is performed in that document directly. **With this correction, the Reasoning Token Architecture program (Phases 1, 2, 3A, 3B, 3C) is complete.**

**Correction Notice (Atlas UX Architecture Missing Foundation Resolution Phase 1: UX-012D Canonical Foundation Additions task — 2026-08-02):** This is a later, additive correction; it does not revise, replace, or reopen any notice above. The completed Cross-Document Token Consistency Audit and the subsequent Missing Foundation Token Resolution investigation (both 2026-08-02) identified nine token gaps left disclosed, not resolved, by the completed Foundation & Collaboration Token Alignment task. Five are closed here; the remaining four (focus-ring offset, `progress.fill`, `progress.track`, `loading.skeleton.base`) are confirmed, by that same investigation, to require either no addition or a Product/Design decision this document's own governance process does not itself have the authority to make, and remain open. Two neutral border weights — `color.border.hairline` and `color.border.standard` — are added to Section 3, below: both were already fully described conceptually in Section 2's own "Border tokens" sentence (hairline: 1px, subtle section dividers; standard: 1px at higher opacity, container boundaries) but never given literal tokens — exactly the same gap Phase 1 closed for the Text and Surface Hierarchies. Three motion events — Open, Close, and Navigate — are added to the existing Motion Architecture group, below, extending (not replacing) the Phase 3B addition: Phase 3B's own notice excluded these three, alongside Replace, on the stated basis that "no current consuming reference" existed at the time; `UX-013A-Atlas-Component-Specification-Foundation-Components.md`'s own completed Foundation & Collaboration Token Alignment task (2026-08-02) subsequently established genuine, live consuming references for all three (Dialog open/close; Navigation Bar and Scroll Container auto-scroll) — Replace alone remains unreferenced and unformalized. Duration and easing values for Open and Close are taken directly from `UX-012C`'s own "Workspace transition... Duration: standard (200–250ms)" statement and each event's own already-stated easing (Open: ease-out; Close: ease-in); Navigate's easing is taken directly from `UX-012C`'s own "Easing: ease-in-out for the scroll curve," but its duration is not aliased to a single shared category — `UX-012C` itself states Navigate's duration "depends on scroll distance," a genuinely two-valued, contextual property unlike every other event in this group, and none is invented to force a single value. **All additions in this correction reuse existing categories and, for Motion, the existing shared duration/easing foundation — no new top-level token category is introduced, no existing token's own value or meaning is changed, and no Product semantic, component architecture, ownership, lifecycle, interaction, or accessibility behavior is altered anywhere in this document.** `focus.ring.offset`, `progress.fill`, `progress.track`, and `loading.skeleton.base` remain unresolved, exactly as the governing investigation left them — none is added here. The corresponding `UX-013A` remap for the five additions in this correction is a separate, later task.

**Correction Notice (Atlas UX Architecture Token Architecture Release Polish (Final Sprint) task — 2026-08-02):** This is a later, additive correction; it does not revise, replace, or reopen any notice above. The completed Final Corpus Verification (Release Candidate Audit) found that the Motion Architecture group's own introductory sentence, below, still read "Only these seven of the twelve named UX-012C motion events are instantiated here... Open, Close, Replace, and Navigate have no current consuming reference," even though the Missing Foundation Resolution Phase 1 task's own extension, immediately following in the same group, had already instantiated Open, Close, and Navigate — a literal, verifiable self-contradiction within this document's own canonical text. Prior text, quoted verbatim: *"Only these seven of the twelve named UX-012C motion events are instantiated here — Open, Close, Replace, and Navigate have no current consuming reference and are not tokenized speculatively; a future consuming document that needs one requests it through this document's own governance process rather than inferring a plausible-looking name."* (This sentence had itself already been annotated, not rewritten, by the Missing Foundation Resolution Phase 1 notice above — that annotation is superseded by this correction, which rewrites the sentence directly.) The sentence is corrected below to state the current architecture directly, without relying on a reader following a forward pointer to reconcile it. **This correction changes no token, no duration, no easing, and no event's own resolution — Replace remains the sole unformalized event, exactly as before. It is wording only.** No new top-level category, no new shared duration category, no new shared easing reference, and no Product semantic, component architecture, ownership, lifecycle, or accessibility behavior is introduced or altered.
...
Text Hierarchy *(added per the Reasoning Token Architecture Phase 1: UX-012D Foundations task, 2026-08-02)*: the neutral text-color hierarchy this document's own Section 2 already describes conceptually ("text levels — primary, secondary, tertiary, dim") but never gave literal tokens beyond one naming-convention example. Values: `color.text.primary` (normal, high-emphasis body/content text — the same token already named as this document's own naming-convention example, Section 4; this entry gives it its own semantic-group definition, not a duplicate), `color.text.secondary` (supporting, less-prominent, or metadata-adjacent text), `color.text.tertiary` (quiet, contextual, or muted text). This group names a semantic role only — it defines no fixed component, no authorship, no authority, no warning, and no lifecycle meaning; consuming documents map their own local text uses to these three roles. It is distinct from typography tokens (`type.*`), which govern scale, weight, and line height, not color. The fourth conceptually-named level, "dim," is not formalized here and remains open for a future, separately-justified addition.

Surface Hierarchy *(added per the same task)*: the neutral surface hierarchy this document's own Section 2 already describes conceptually ("four background surface levels — background, surface, elevated, panel") but never gave literal tokens. Values: `surface.background` (the base Workspace surface), `surface.primary` (the standard content surface — named "primary" here, rather than "surface," to avoid a `surface.surface` collision; this is the same conceptual level Section 2 calls "surface"), `surface.elevated` (a raised or distinguished surface, e.g., a strong container), `surface.panel` (a bounded, secondary surface, e.g., a side panel or context panel). Existing component-scoped surface tokens (`surface.decision.card`, `surface.conclusion.container`, `surface.historical`) are unchanged and remain their own, more specific values; where a future component-scoped surface is genuinely needed, it should reference or alias one of these four neutral foundations rather than introduce an unrelated name.

Canonical Reasoning Token Mapping *(added per the Reasoning Token Architecture Phase 3A: Canonical Reasoning Token Mapping task, 2026-08-02)*: this is a mapping contract, not a new semantic group and not a component — it states how Reasoning-tier component presentation (Conclusion, Supporting Factors, Challenges, Assumptions, Opportunity Summary, Opportunity Cost, Scenario Analysis, Recommendation/Proposed Decision Candidate Content, Reasoning Block) uses the token categories already established above, rather than inventing its own. Reasoning component typography uses `type.*` — already fully governed by `UX-013B`'s own generic `type.role1.*` through `type.role5.*` system; this mapping contract does not touch typography and adds no new typography scale, weight, or line-height value. Reasoning component text color uses `color.text.*` — specifically, the Text Hierarchy group, above; component-specific meaning (which Reasoning component, which field) belongs in the surrounding prose and the component's own specification, never encoded as a new top-level token category. Where two components share the same visual role (an item's own identifying name, a supporting statement, a structural label), they reuse the same Text Hierarchy token — a name field in one Reasoning component is not given a different token from a name field in another merely because the components have different names. Aliases, where used, are documented explicitly in the traceability table each correction using this contract maintains. Wildcard notation (`category.role.*`) is not a canonical token identifier — every canonical Reasoning-tier token names one exact, complete leaf value. This contract renders Reasoning content; it does not define the Product Concept Reasoning, does not create new Conclusion variants, does not merge Atlas Recommendation with Proposed Decision Candidate Content (per `ADR-003`), does not alter Scenario Analysis's own content ownership (per `ADR-004`), and does not encode epistemic importance, truth, Confidence, or authority — a token's own color tier reflects visual emphasis only. Local implementation names invented by a consuming document do not become canonical merely by consistent local use; only a token defined in this document, under an existing category, is canonical.
...
Monitoring: tokens for monitoring condition presentations. Values: `color.semantic.amber` (for Approaching state), `color.border.monitoring-rule` (the left-border rule on monitoring items), `type.status.monitoring`. *(Added per the Status Badge Token Architecture Correction task, 2026-08-02: `color.semantic.red` — category: color; subcategory: semantic; role: red — for Triggered state. A restrained red semantic accent, parallel to `color.semantic.amber`, representing a condition of elevated severity beyond what amber communicates. This formalizes, as a token, the visual escalation already described in prose at `UX-012B` §6 ("Broken" Assumption: "restrained red border") and §9 (Invalidation Trigger: "amber treatment transitions to restrained red") — no new visual meaning is introduced, only a name for an escalation this document's own text already required. Usable for Monitoring Triggered and other genuinely equivalent high-severity states; it does not encode Product lifecycle meaning of its own — it is a color reference only, applied to whatever fact a component already renders.)* *(Added per the Reasoning Token Architecture Phase 1: UX-012D Foundations task, 2026-08-02: `color.semantic.green` — category: color; subcategory: semantic; role: green — a restrained Atlas green, parallel to `color.semantic.amber` and `color.semantic.red`, completing the third of the four semantic accent colors this document's own Section 2 already names ("amber, green, red, blue") but had not yet each been given a literal token. Formalizes the visual treatment already described in prose at `UX-012B` §6 ("Holding" Assumption: "green border"). Usable for Assumption Holding and other genuinely equivalent positive/holding states. `color.semantic.green` is not a success score, not a Decision Quality signal, not a positive Outcome judgment, and not an ownership or approval signal — like every other semantic accent color in this group, it is a color reference only, rendering a state a component already establishes elsewhere; text and programmatic labels remain the authoritative signal, per `UX-012A` §15 and this component's own Accessibility requirements. `color.semantic.blue`, the fourth named accent, is not formalized here — no source examined during the governing investigation identified a concrete use requiring it, and it is not added speculatively.)*
...
Completed: tokens for the post-recording state — the completed Final Decision Card, the cleared body, the recorded state. Values: `surface.decision.card` at its full recorded visual authority (no visual change from the draft state — the authority comes from the populated content, not a different surface treatment). *(Added per the Status Badge Token Architecture Correction task, 2026-08-02: `type.badge.completed` — category: type; role: badge; variant: completed — the text-emphasis token for a compact Completed/Recorded Status Badge label specifically, distinct from the full Final Decision Card treatment above. No dedicated background token is defined for this badge, consistent with this group's own already-stated principle that "the authority comes from the populated content, not a different surface treatment" — the badge renders on the Status Badge component's own neutral default surface. This token communicates recorded/completed status textually; it does not itself imply success, quality, or a positive Outcome, per `APS-005`'s own Decision Quality boundary, already respected throughout this document.)*
...
Editing *(added per the Reasoning Token Architecture Phase 1: UX-012D Foundations task, 2026-08-02)*: tokens for the document-dimming overlay presented while a field is being edited. Values: `opacity.editing.overlay` (0.05–0.08) — formalizing, as a literal token, the range this document's own Section 2 already states for the editing interaction state ("editing (document dimming overlay opacity: 0.05–0.08)"). This is an opacity value only, applied to whatever surface a component already renders during editing — it does not itself represent a surface or background color, does not imply a disabled, unavailable, or loading state, and does not alter what fields are editable or when.

Focus: `color.focus.ring`, `width.focus.ring`, `radius.focus.ring` — the three parameters of the focus ring, constant across all surfaces. **This is the canonical naming order** *(confirmed per the Reasoning Token Architecture Phase 1: UX-012D Foundations task, 2026-08-02, resolving this document's own prior internal inconsistency against Section 15, Accessibility Governance, which previously stated the reverse order — see that section, below, for the corrected text)*: `color.*`, `width.*`, and `radius.*` are this document's own established leading categories for every other value-typed token (`color.text.*`, `color.border.*`, `color.semantic.*`; no other token in this document uses `focus.*`, or any object name, as its own leading category), and Section 3 is this document's own definitional section for token identity, governing over Section 15's own consuming reference to it.

Motion Architecture *(added per the Reasoning Token Architecture Phase 3B: Motion & Interaction Foundations task, 2026-08-02)*: instantiates, as literal tokens, the duration-category and easing-reference model this document's own Section 2 already describes conceptually for the twelve UX-012C motion events. Shared foundation — reused by every event token below, never duplicated: `motion.duration.immediate` (0ms), `motion.duration.brief` (100–150ms), `motion.duration.standard` (200–250ms), `motion.duration.deliberate` (350–400ms); `motion.easing.out`, `motion.easing.in`, `motion.easing.in-out`. Event tokens — each an alias naming one shared duration category and one shared easing reference, not an independent value: `motion.expand.duration` (`motion.duration.standard`) / `motion.expand.easing` (`motion.easing.out`) — a structural reveal, decelerating into view; `motion.collapse.duration` (`motion.duration.standard`) / `motion.collapse.easing` (`motion.easing.in`) — the reverse, accelerating closed; `motion.insert.duration` (`motion.duration.brief`) / `motion.insert.easing` (`motion.easing.out`) — a list item appearing; `motion.remove.duration` (`motion.duration.brief`) / `motion.remove.easing` (`motion.easing.in`) — a list item disappearing; `motion.highlight.duration` (`motion.duration.brief`) / `motion.highlight.easing` (`motion.easing.in-out`) — a single attention pulse on a cross-component navigation target, not a persistent state (a reduced-motion context's own persistent "focused" background substitute is component-level specification, not a Motion token of its own); `motion.update.duration` (`motion.duration.brief`) / `motion.update.easing` (`motion.easing.out`) — a one-shot cue that a value changed since the last session; `motion.fade.duration` (`motion.duration.brief`) / `motion.fade.easing` (`motion.easing.out`) — a dismissal fade, distinct in meaning from Remove even where its duration and easing values happen to be shared. Ten of the twelve named UX-012C motion events are instantiated in this group — Expand, Collapse, Insert, Remove, Highlight, Update, and Fade above, and Open, Close, and Navigate in the extension below. Only Replace has no current consuming reference and is not tokenized speculatively; a future consuming document that needs one requests it through this document's own governance process rather than inferring a plausible-looking name. Loading is deliberately excluded from this group — see the Loading group, above, which already governs the identical effect via `opacity.loading.pulse.min`/`.max`; a second Motion token for the same effect would be Token Duplication (Section 12). Every token in this group is subject to the reduced-motion requirement already stated in Section 14 ("All motion must reference the reduced-motion flag and produce the instantaneous variant when the flag is active") — this group adds no separate reduced-motion rule of its own, and a consuming document's own reduced-motion fallback behavior (for example `UX-013B`'s own Reduced Motion section) is component-level specification, not a Motion token.

**Extension** *(added per the Missing Foundation Resolution Phase 1: UX-012D Canonical Foundation Additions task, 2026-08-02)*: the sentence above originally read, in full: "Only these seven of the twelve named UX-012C motion events are instantiated here — Open, Close, Replace, and Navigate have no current consuming reference and are not tokenized speculatively." Three of those four — Open, Close, and Navigate — now have a genuine, live consuming reference: `UX-013A-Atlas-Component-Specification-Foundation-Components.md`'s own completed Foundation & Collaboration Token Alignment task (2026-08-02) disclosed that its Dialog component uses Open/Close on enter/exit, and its Navigation Bar and Scroll Container use Navigate for auto-scroll — exactly the forcing function the original sentence found absent. Three further event tokens are instantiated on that basis, using the identical shared-alias model, grounded directly in `UX-012C` §10 and its own preceding Motion Categories discussion: `motion.open.duration` (`motion.duration.standard`) / `motion.open.easing` (`motion.easing.out`) — per `UX-012C`'s own "Workspace transition... Duration: standard (200–250ms)" (§10's own preceding Motion Categories discussion) and Open's own §10 entry, "Easing: ease-out"; `motion.close.duration` (`motion.duration.standard`) / `motion.close.easing` (`motion.easing.in`) — the same Workspace-transition duration statement, which covers close in the reverse direction, and Close's own §10 entry, "Easing: ease-in"; `motion.navigate.easing` (`motion.easing.in-out`) — per `UX-012C` §10's own Navigate entry, "Easing: ease-in-out for the scroll curve." `motion.navigate.duration` is not aliased to a single shared category: `UX-012C`'s own governing text (its Motion Categories discussion, "Navigation") states duration for Navigate "depends on scroll distance; brief for short scrolls, standard for long scrolls" — a genuinely two-valued, contextual property, unlike every other event in this group. `motion.navigate.duration` is therefore defined as context-dependent between `motion.duration.brief` (short scrolls) and `motion.duration.standard` (long scrolls); no scroll-distance threshold separating "short" from "long" is stated by any governing source, and none is invented here — a consuming document applies whichever value matches its own scroll distance, per `UX-012C`'s own description, and discloses the threshold it uses, if any, in its own specification. Replace remains the sole named UX-012C motion event with no current consuming reference and is not tokenized speculatively, unchanged from the original sentence above. This extension adds no new top-level category, no new shared duration category, and no new shared easing reference — every value above reuses the four duration categories and three easing references already instantiated at the top of this group. It changes no Product semantic, component architecture, ownership, lifecycle, or accessibility behavior. The corresponding `UX-013A` remap is a separate, later task.

Interaction *(added per the same task)*: instantiates, as a literal token, the hover treatment this document's own Section 2 already describes conceptually ("hover (surface lightening delta)") but never gave a token. Value: `opacity.interaction.hover` — the lightening delta applied to whatever surface a hoverable component area already renders, following the same pattern already established for `opacity.editing.overlay` (Editing group, above). This is a delta value, not a fixed background color or a surface of its own — a consuming document that names a specific background color for a hover state has misdescribed this token. `interaction.*` is not adopted as a token category here or anywhere in this document — hover, like focus, is a role named beneath an already-established category (`opacity.*`), not a category of its own, following the same naming-order precedent already set for Focus, above, and the closed top-level category list (Section 4). No specific numeric delta is established by any governing source examined during the governing task; none is invented here — a future implementation sets the underlying global value, and this token's own semantic name and role do not change if that value is later tuned. The remaining thirteen UX-012C interaction states are unaffected by this addition and remain governed by their own already-existing tokens or, where no consuming document yet requires one, unformalized.

Canonical Attribution & Action Text Mapping *(added per the Reasoning Token Architecture Phase 3C: Attribution & Action Text task, 2026-08-02)*: a mapping contract, not a new semantic group — states how attribution labels and action-affordance text reuse the Text Hierarchy (above) and `UX-012B` §15's own already-adopted Action Components emphasis taxonomy, rather than inventing a new token family. Attribution labels (the stated source of a piece of content — Atlas-origin or user-origin) use `color.text.secondary`, matching the already-established treatment of the sibling origin label wherever both appear together in a consuming document. Action-affordance text follows `UX-012B` §15's own explicit emphasis rules by role: Primary Action ("the single dominant action... defined surface or clear outline at primary text color") uses `color.text.primary`; Inline Action and Section Action — both explicitly Tertiary emphasis; Inline Action's own named examples include "Edit," "Dismiss," and navigation-style links — use `color.text.tertiary`. `UX-012B`'s own distinct "Secondary Action" (a footer-level action placed alongside a Primary Action, e.g. "Save Draft") is a separate, footer-scoped concept; a consuming document's own local action name that happens to contain the word "secondary" is not, on that basis alone, a Secondary Action in `UX-012B`'s own sense — its actual placement and role, not its local name, governs which tier applies. This mapping renders attribution and action-affordance presentation; it does not itself define authorship, provenance, or acceptance behavior, which remain governed by `ADR-002` C-02 and its addendum and by `ADR-003`'s own Accept/Modify/Decline mapping to the generic Atlas Suggestion interaction pattern. Local implementation names invented by a consuming document do not become canonical merely by consistent local use; only a token defined in this document, under an existing category, is canonical.

Border *(added per the Missing Foundation Resolution Phase 1: UX-012D Canonical Foundation Additions task, 2026-08-02)*: instantiates, as literal tokens, two of the three neutral border weights this document's own Section 2 already describes conceptually ("Border tokens: Three border weights — hairline..., standard..., semantic-rule...") but never gave literal tokens for. Values: `color.border.hairline` (1px, the subtlest structural separator — used for subtle section dividers, a step above the surface color, a step below text colors), `color.border.standard` (1px at higher opacity — used for container boundaries, where two surfaces of close tonal value are adjacent and need a firmer separation than a hairline provides). The third weight this same Section 2 sentence names, "semantic-rule," is not formalized here — it is already fully governed by the existing Contradiction group's `color.border.contradiction.*` tokens and `color.border.monitoring-rule`, above, which together already instantiate the semantic-rule weight for every state that currently requires one; a third, generic `semantic-rule` token would duplicate them, an anti-pattern this document's own Section 12 already names (Token Duplication). These two tokens name a neutral structural role only — they carry no semantic-state meaning, no severity, no warning, and no Product lifecycle meaning; a consuming document that needs a border to communicate a state (a severity tier, a monitoring condition) uses the Contradiction or Monitoring groups instead, never these two. `UX-013A-Atlas-Component-Specification-Foundation-Components.md`'s own completed Foundation & Collaboration Token Alignment task (2026-08-02) disclosed eight structural-border occurrences and three hairline-tier Divider occurrences with no canonical token to cite; these two additions close that gap. The corresponding `UX-013A` remap is a separate, later task.
...
Token governance clarifications *(added per the Reasoning Token Architecture Phase 1: UX-012D Foundations task, 2026-08-02, in response to token-naming defects found in downstream consuming documents)*:
— **Top-level token categories are closed unless amended through this document's own governance.** The established categories are exactly those named in Section 2 and instantiated in Section 3 (`color.*`, `space.*`, `type.*`, `surface.*`, `motion.*`, `opacity.*`, `width.*`, `radius.*`, `accessibility.*`, `cursor.*`). A consuming document may not introduce a new top-level category (for example, a category named after a component tier, such as `reasoning.*` or `conclusion.*`, or after a UI-pattern name, such as `status.*` or `editing.*`) merely because it needs a token for a concept those categories don't yet name.
— **Component-specific meaning belongs in the role/variant portion of a token, beneath an existing category** — for example `type.decision.statement`, `surface.historical`, `color.text.historical` — never as the category itself.
— **A conceptual category description in Section 2 does not, by itself, authorize an inferred literal leaf token.** Every literal token this document treats as canonical is individually named in Section 3 or Section 4; a consuming document that needs a token for a concept only described conceptually here must request its formal addition, not infer a plausible-looking name and treat it as already canonical.
— **Aliases must be explicitly documented**, following the pattern already established for Warning ("The Warning semantic group maps to the same amber tokens" as Contradiction, Section 3) — an alias is a stated equivalence between a semantic group and an existing token, not a private, undocumented assumption.
— **Wildcard notation (`category.role.*`) is not a canonical token identifier.** Every canonical token in this document names one exact, complete leaf value; a consuming document's own summary or reference table may group several exact tokens together for readability, but the group heading itself is not a token and must not be cited as if it were one.
— **Typography tokens (`type.*`) and text-color tokens (`color.text.*`) are distinct categories and are never conflated.** Scale, weight, and line height are typography; color is a separate, independent property, even when applied to the same text.
— **Tokens render state; they do not define it.** A token's own existence or name never constitutes the Product- or UX-layer definition of the state it renders — that definition belongs to the governing source (`UX-012`, `UX-012B`, or, where applicable, Product Architecture) named wherever the token is used.
...
— The focus ring must reference focus tokens (`color.focus.ring`, `width.focus.ring`, `radius.focus.ring`). *(Corrected per the Reasoning Token Architecture Phase 1: UX-012D Foundations task, 2026-08-02: this line previously read "`focus.ring.width`, `focus.ring.color`, `focus.ring.radius`," conflicting with Section 3's own canonical ordering. Section 3, this document's own definitional section for token identity, governs; see the Correction Notice above and Section 3's own Focus group, below.)*

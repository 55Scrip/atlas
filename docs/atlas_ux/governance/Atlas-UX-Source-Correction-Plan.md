...
**9 of 18 files require no correction. 9 files require direct or authority correction**, matching ADR-002's own "at minimum" list, with three additions surfaced after this plan's original authoring and folded into already-accepted decisions without reopening any of them: UX-009's hard-blocking Challenge rule and UX-010's bare four-field restatement (both C-04, surfaced during this planning pass's own full read), and UX-011's non-canonical Decision Workspace terminology (C-03, surfaced by a later full-file reassessment and corrected via Phase 2A — see Section 25). (This plan's original text stated "11 of 18... 7 files"; recounting the table above against its own actual per-row classifications, including UX-011's now-corrected row, gives 9/9 — see Section 25.)
...
| C-03 | Canonical 13-section order, adopted names | UX-012 (§17 order + naming), UX-011 (naming only — found by later reassessment, corrected via Phase 2A; see Section 25) | UX-009, UX-009A, UX-010 (order only — matches exactly) | UX-012 (order), UX-009/UX-009A/UX-010/UX-011 (naming only) | UX-013B (validation only) | UX-012 | Exactly one section order appears anywhere in the corpus; Sections 5/6/12 use identical canonical names everywhere, including UX-011 as of Phase 2A |
...
**This plan preserves:** Proposed Decision remaining early (position 3) as a testable working hypothesis; no standalone "What Changed" section anywhere in the Decision Workspace sequence; the canonical 13-section sequence exactly as UX-009/UX-009A/UX-010/UX-011 now consistently state it (UX-011 corrected via Phase 2A, per Section 25) — the sequence itself is not redesigned anywhere in this plan, only renamed in the four documents that used non-canonical prose names for Sections 5/6/12 (UX-009, UX-009A, UX-010, UX-011) and corrected in order in the one outlier document (UX-012).
...
|---|---|---|---|---|
| "What Supports This Decision" | Supporting Factors | UX-009, UX-009A, UX-010, UX-011 (UX-011 corrected later, via Phase 2A — see Section 25) | Matches the actual named Reasoning component (UX-013B) that populates this section | Yes — a corrected document's own amendment note may quote the original name verbatim as historical record |
| "What Challenges This Decision" | Challenges | UX-009, UX-009A, UX-010 | Same reason | Yes, same basis |
| "Final Decision Summary" (and its abbreviated form, "Final Summary," and "Final Decision Summary card") | Final Decision Card | UX-009, UX-009A, UX-010, UX-011 (UX-011 corrected later, via Phase 2A — see Section 25) | Matches UX-012/UX-012B's already-established component name | Yes, same basis |

...
**Correction Notice (Phase 6D, governed by `ADR-003-Recommendation-Identity-and-Terminology-Resolution.md` — 2026-07-30):** This document's own two passages relaying `UX-013F-Foundation-Reasoning-Component-Library-Assembly.md`'s "Recommendation" component name by citation are updated to match that document's own Phase 6D correction of the same component to **Proposed Decision Candidate Content** (short form: "Candidate Content"). Neither passage independently asserted the component's name; both merely relayed it via an explicit `(UX-013F)` citation, and this correction updates only the relayed name, without otherwise rewriting either passage. Prior text, quoted verbatim: "Atlas Recommendation Presentation claimed as an authorship configuration of Reasoning's own Recommendation component (UX-013F), not a second component." (line ~74); "`AtlasRecommendationPresentation` (UX-013D) | `Recommendation` (`UX-013F`) with `isAtlasGenerated={true}` | Component → Props on an existing UX-013F component | Atlas recommendation authorship is expressed through existing Recommendation props" (line ~124, the `UX-013F`-tagged component-name cell and the trailing "existing ... props" reference only). ADR-003 reserves "Atlas Recommendation" exclusively for Concept A (`UX-012-Atlas-Design-System-Workspace-Consistency-Specification.md` §28), unrelated to this document's own `AtlasRecommendationPresentation` label or to UX-013F's corrected component, and unaffected by this correction. The `AtlasRecommendationPresentation` label itself, its own `(UX-013D)` attribution, and this document's own three-tier provenance classification of it remain entirely unchanged — only the relayed component name is corrected. No new component, variant, or Domain Object is introduced by this correction. This notice does not claim the corrected wording existed in this document's original, historical version — the prior wording is preserved verbatim, in quotation, above. All content outside these two corrected passages is unchanged.
...
- Atlas Recommendation Presentation claimed as an authorship configuration of Reasoning's own Proposed Decision Candidate Content component (UX-013F), not a second component.
...
| `AtlasRecommendationPresentation` (UX-013D) | `Proposed Decision Candidate Content` (`UX-013F`) with `isAtlasGenerated={true}` | Component → Props on an existing UX-013F component | Atlas recommendation authorship is expressed through existing Candidate Content props |

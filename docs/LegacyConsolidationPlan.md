...
**Status:** `atlas/analysis/` CLOSED Sprint 141. `atlas/decision/` CLOSED Sprint 144. Provider boundary CLOSED Sprint 146. Portfolio boundary CLOSED Sprint 148. Evidence package CLOSED Sprint 150. Reasoning package CLOSED Sprint 153. Risk package CLOSED Sprint 155. Principles package CLOSED Sprint 158. Comparison package CLOSED Sprint 160. Home package CLOSED Sprint 162 (Sprint 162 confirmed Sprint 161 findings unchanged — no cleanup warranted). Sprint 163 RC checkpoint: all 10 closed tracks verified stable — deleted modules absent, active modules importable, retired CLI commands retired, provider boundaries unchanged, RC2 green. Sprint 164 audited `atlas/intelligence/`: package is clean, all 5 exports active, no stale imports, no Blueprint successor, no cleanup warranted. Intelligence package CLOSED Sprint 165. Sprint 166 audited `atlas/conversation/`: package is clean, all 6 exports active, no stale imports, no Blueprint successor, no cleanup warranted. Conversation package CLOSED Sprint 167. Sprint 168 audited `atlas/dashboard/`: package is clean, all 6 exports active, cleanest provider boundary audited (no concrete provider imported), no stale imports, no Blueprint successor, no cleanup warranted. Dashboard package CLOSED Sprint 169 (Sprint 169 confirmed Sprint 168 findings unchanged — cleanest provider boundary audited, no cleanup warranted). Sprint 170 audited `atlas/capabilities/portfolio_intelligence/` (note: `atlas/portfolio_intelligence/` does not exist as a top-level package — legacy `PortfolioIntelligenceEngine` was deleted Sprint 128, `atlas.analysis.portfolio` deleted Sprint 135): package is architecturally exemplary, 4 exports active, cleanest dependency surface of any capability (depends only on `atlas.shared`), no stale imports, no provider coupling, no cleanup warranted. Portfolio intelligence capability CLOSED Sprint 171 (Sprint 171 confirmed Sprint 170 findings unchanged — stale docstring removed from `PortfolioFitInput`, no runtime cleanup warranted). Sprint 172 RC checkpoint: all 14 closed tracks verified stable — deleted modules absent, active modules importable, retired CLI commands retired, provider boundaries unchanged, RC2 green, 1524 tests passed. Sprint 173 audited `atlas/cli/` deprecated command registry: `_REGISTRY` empty, all 7 retired commands accurate, no retired command callable, all retirement metadata verified accurate after 14 closures. One cleanup candidate: 3 empty shell app groups (`evidence_app`, `reason_app`, `risk_app`) registered but exposing zero commands — residual scaffolding from retired commands, confusing to users. CLI cleanup track CLOSED Sprint 174 — removed 3 empty shell app groups (`evidence`, `reason`, `risk`) from `atlas/cli/main.py`; `atlas --help` no longer shows empty groups; all active commands and retired command metadata unchanged; 1536 tests passed. Sprint 175 RC checkpoint: all 15 closed tracks verified stable — deleted modules absent, active modules importable, empty CLI groups absent from help surface, retired CLI commands not callable, provider boundaries unchanged, RC2 green, 1541 tests passed. Sprint 176 audited `atlas/capabilities/`: 5 subpackages (4 active + 1 closed Sprint 171), 52 total active exports, no stale imports, no provider coupling, no circular dependencies, no cleanup warranted. See `docs/CapabilitiesCleanupPlan.md`. Sprint 177 audited `atlas/domains/`: 9 subpackages, ~1,730 lines, 68 total active exports, no stale imports, no provider coupling, no upward dependencies, boundary direction correct throughout, no cleanup warranted. See `docs/DomainsCleanupPlan.md`. Sprint 178 audited `atlas/adapters/`: 5 adapter modules, 756 lines, 7 public symbols, all active with production callers, no stale imports, no provider coupling, no upward dependencies, portfolio boundary CLOSED Sprint 148 verified stable, `atlas.analysis.scores.clamp_score` confirmed active utility (not stale), no cleanup warranted. See `docs/AdaptersCleanupPlan.md`. Sprint 179 audited `atlas/analysis/` (the legacy company analysis runtime surface — `atlas/company_analysis/` does not exist as a standalone package): 5 active modules, ~655 lines, 12 active `__all__` exports, Sprint 141 closure verified (12 deleted modules remain absent), capability boundary with `atlas/capabilities/company_analysis/` confirmed clean (capability does not import legacy analysis), one stale alias found (`CompanyAnalysisProvider` in `atlas/analysis/company_analysis.py:154` — zero external callers, not in `__all__`). See `docs/CompanyAnalysisCleanupPlan.md`. Sprint 180 removed stale `CompanyAnalysisProvider` alias from `atlas/analysis/company_analysis.py` (zero external callers, not in `__all__`, targeted company-analysis residual cleanup found during Sprint 179 — does not reopen Sprint 141 closure); all 12 active `__all__` exports preserved; 1598 tests passed, RC2 green. Sprint 181 RC checkpoint: company analysis residual cleanup verified, all 15 closed tracks stable, all 13 deleted modules absent, `CompanyAnalysisProvider` absent from all active code, all 12 active `atlas.analysis` exports importable, capability boundary clean, CLI boundaries correct, provider boundaries unchanged, 15 active packages importable, 1598 tests passed, RC2 green, demo passes. Sprint 182 audited `atlas/capabilities/company_analysis/`: 4 modules, 571 lines, 9 `__all__` exports — all active, legacy/capability boundary clean (no cross-imports in either direction), zero provider coupling, zero stale imports, fully Blueprint-aligned, cleanest provider boundary of any capability, no cleanup warranted. See `docs/CompanyAnalysisCapabilityCleanupPlan.md`. Company analysis capability cleanup track CLOSED Sprint 183 — Sprint 183 confirmed Sprint 182 findings unchanged; all 9 exports active and importable, boundaries clean, 1605 tests passed, RC2 green. Sprint 184 audited `atlas/decision_journal/`: 2 modules, 605 lines, 11 `__all__` exports — all active, 4 lateral legacy-layer dependencies (evidence, language, principles, profile) all intentional and runtime-active, no provider coupling, no CLI coupling, no stale imports, JSON persistence is injected-path and deterministic, `language_report` not-serialized pattern is intentional, no cleanup warranted. See `docs/DecisionJournalCleanupPlan.md`. Decision journal cleanup track CLOSED Sprint 185 — Sprint 185 confirmed Sprint 184 findings unchanged; all 11 exports active and importable, 4 lateral dependencies all intentional, provider boundary clean, persistence intentional, no stale imports, 1614 tests passed, RC2 green. Sprint 186 audited `atlas/watchlist_review/`: 2 modules, 894 lines, 11 `__all__` exports — all active, active CLI callers (`atlas watchlist review`), active application callers (`atlas/home/engine.py`, `atlas/conversation/engine.py`), wide lateral dependency footprint (10 sibling packages) all intentional, one cleanup candidate: `atlas.providers` imported directly in `engine.py` (non-adapter, non-CLI module — `CompanyDataProvider` field type, `MockCompanyAnalysisProvider` default). No stale imports from closed tracks. `WatchlistEngine` correctly absent (Sprint 94 guardrail). Deprecation note in `atlas/cli/deprecations.py` refers to `atlas.evidence` engine deletion deferral, not `watchlist_review` engine deletion — note remains accurate. See `docs/WatchlistReviewCleanupPlan.md`. Watchlist review cleanup track CLOSED Sprint 187 — Sprint 187 assessed the provider boundary issue: `CompanyDataProvider` is already a Protocol (type-only import); `MockCompanyAnalysisProvider()` is instantiated as the deterministic default (required for "works with no configuration" behavior); same pattern used in `atlas/cli/main.py` and `atlas/home/engine.py`; classified as acceptable legacy coupling; no code change; 1622 tests passed, RC2 green. Sprint 188 RC checkpoint: all 17 closed tracks + residual cleanup verified stable; decision journal (Sprint 185) and watchlist review (Sprint 187) closures confirmed; watchlist review provider coupling classified as acceptable legacy coupling; 17 active packages importable; deleted modules absent; retired CLI commands non-callable; empty CLI groups absent; 1622 passed, 3 skipped; RC2 green; demo passes. Sprint 189 audited `atlas/watchlist/` package (note: no standalone `atlas/watchlist/` package exists — watchlist surface is distributed across `atlas/capabilities/watchlist_intelligence/` (545 lines, 13 exports) and `atlas/adapters/watchlist.py` (198 lines, 2 public functions)): both locations are Blueprint-aligned, provider-free, no stale imports, no provider coupling, no CLI coupling, no upward dependency on watchlist_review, legacy `atlas/analysis/watchlist.py` remains deleted, `WatchlistEngine` remains absent, 13 exports active and importable, adapter functions active — no cleanup warranted. See `docs/WatchlistCleanupPlan.md`. Watchlist cleanup track CLOSED Sprint 190 — Sprint 190 confirmed Sprint 189 findings unchanged; all 13 watchlist_intelligence exports active and importable, adapter functions active, production callers unchanged (11 files), no stale imports, no provider coupling, no CLI coupling, no upward dependency on watchlist_review, legacy atlas/analysis/watchlist.py remains deleted, WatchlistEngine remains absent, boundary directions correct throughout; 1637 tests passed, RC2 green, demo passes. Sprint 191 RC checkpoint: all 20 closed tracks verified stable; three recent closures (decision journal Sprint 185, watchlist review Sprint 187, watchlist Sprint 190) confirmed unchanged; 13 deleted modules absent; 7 retired CLI commands non-callable; empty CLI groups absent; 18 active packages importable; provider boundaries unchanged or intentionally classified; watchlist review provider coupling remains Outcome B (acceptable legacy coupling); 1637 passed, 3 skipped; RC2 green; demo passes. Sprint 192 audited `atlas/analysis/` active residual surface (note: this is NOT a reopening of Sprint 141 — it audits the active runtime surface preserved by Sprint 141): 5 surviving modules, 652 lines, 12 `__all__` exports (all active), `clamp_score` shared utility (11 cross-package callers), capability boundary clean (no cross-imports in either direction with `atlas/capabilities/company_analysis/`), domains boundary clean, no stale imports from deleted modules, no network access, one low-priority cleanup candidate (3 zero-caller provider re-exports in `__init__.py`). See `docs/AnalysisResidualCleanupPlan.md`. Residual analysis cleanup track CLOSED Sprint 193 — Sprint 193 confirmed Sprint 192 findings unchanged, removed 3 zero-caller provider re-exports (`CompanyDataProvider`, `MockCompanyAnalysisProvider`, `YahooFinanceProvider`) from `atlas/analysis/__init__.py` and `__all__` (12→9 exports); no runtime behavior changed; provider selection unchanged at CLI layer; guardrail tests updated; 1648 tests passed, RC2 green, demo passes. Sprint 194 RC checkpoint: all 21 closed tracks verified stable; `atlas.analysis.__all__` 9 exports confirmed; 13 deleted modules absent; 7 retired CLI commands non-callable; 18 active packages importable; provider boundaries unchanged or intentionally classified; 1648 passed, 3 skipped; RC2 green; demo passes. Sprint 195 recommended: audit `atlas/config/` package. Sprint 195 audited `atlas/config.py` (note: `atlas/config/` does not exist as a package directory — the configuration surface is a single 6-line module): stdlib-only imports, 3 path constants (`BASE_DIR`, `DATABASE_DIR`, `DATABASE_PATH`), 1 env var (`ATLAS_HOME`), 2 active production callers (`atlas/database/connection.py`, `atlas/services/database_service.py`), zero provider coupling, zero network access, zero stale imports, clean boundary direction (config ← database ← services ← CLI), `atlas/storage/` does not exist (storage layer is `atlas/database/` + `atlas/services/`). See `docs/ConfigCleanupPlan.md`. Sprint 196 recommended: close config cleanup track. Config cleanup track CLOSED Sprint 196 — Sprint 196 confirmed Sprint 195 findings unchanged; `atlas/config.py` is 6 lines, stdlib-only, zero Atlas imports, zero provider coupling, 2 active callers (`atlas/database/connection.py`, `atlas/services/database_service.py`), `ATLAS_HOME` env var stable and tested; no cleanup warranted; 1654 tests passed, RC2 green, demo passes. Sprint 197 recommended: audit `atlas/database/` and `atlas/services/` together. Sprint 198 recommended: remove zero-caller dead code and close database/services cleanup track. Sprint 197 audited `atlas/database/` (1 module, 20 lines: `connection.py` — `Base`, `get_engine`, `get_session`) and `atlas/services/` (4 modules, 164 lines: `database_service.py`, `company_service.py`, `financial_import_service.py`, `kpi_service.py`) together: all database and service symbols have active production callers except `kpi_service.py` (zero production callers — test-only pure math); schema/ORM gap (6 of 8 schema tables have no ORM model) is intentional; boundary direction correct (config ← database ← models ← services ← CLI); no provider coupling, no network access, no stale imports. Three adjacent zero-caller dead-code items found: `atlas/services/kpi_service.py` (zero production callers), `atlas/models/investment_report.py` (dead re-export shim), `atlas/reports/investment_card.py` (dead function). See `docs/DatabaseServicesCleanupPlan.md`. Sprint 198 removed zero-caller dead symbols (`atlas/services/kpi_service.py`, `tests/test_kpi_service.py`, `atlas/models/investment_report.py`, `atlas/reports/investment_card.py`) and deleted the now-empty `atlas/reports/` directory (no `__init__.py`, zero callers); all active database, services, models, and CLI symbols unchanged; 1671 tests passed, RC2 green, demo passes. Database/services cleanup track CLOSED Sprint 198. Sprint 199 recommended: release candidate checkpoint (22 closed cleanup tracks). Sprint 199 RC checkpoint: all 22 closed tracks verified stable; Sprint 198 removals confirmed absent; database/services exports importable; config/database/services boundary stable; SQLAlchemy/SQLite/schema unchanged; 21 packages importable; 16 deleted modules absent; 7 retired CLI commands non-callable; empty CLI groups absent; provider boundaries unchanged or intentionally classified; 1671 passed, 3 skipped; RC2 green; demo passes. Sprint 200 recommended: audit `atlas/storage/` package. Sprint 200 audited storage boundary: `atlas/storage/` does not exist; zero Python imports of `atlas.storage`; storage/persistence fully owned by `atlas/database/` + `atlas/services/`; config/database/services boundary stable; Sprint 198 removals confirmed absent; 1671 tests passed, RC2 green, demo passes. Storage boundary cleanup track CLOSED Sprint 200. Sprint 201 recommended: release candidate checkpoint (23 closed cleanup tracks). Sprint 201 RC checkpoint: all 23 closed tracks verified stable; Sprint 200 storage boundary confirmed (atlas/storage/ does not exist, zero imports); Sprint 198 removals confirmed absent; database/services exports importable; config/database/services boundary stable; 20 packages importable; 16 deleted modules absent; 7 retired CLI commands non-callable; empty CLI groups absent; provider boundaries unchanged or intentionally classified; 1671 passed, 3 skipped; RC2 green; demo passes. Sprint 202 recommended: audit atlas/models/ package. Sprint 202 audited `atlas/models/` (2 modules, 52 lines): `__init__.py` lazy shim clean (`__all__ = ["Company", "FinancialHistory"]`, no reference to Sprint 198 deletion), `entities.py` contains 2 active ORM models (`Company`, `FinancialHistory`) with 3 production callers (database_service, company_service, financial_import_service), boundary direction correct (models → database, no upward dependency), zero provider coupling, zero stale imports from closed tracks, `atlas/models/investment_report.py` confirmed absent, `atlas/reports/` and `atlas/storage/` confirmed absent; 1671 tests passed, RC2 green, demo passes. Models cleanup track CLOSED Sprint 202 — no cleanup warranted. Sprint 203 recommended: close models cleanup track (confirm Sprint 202 findings unchanged). Sprint 203 confirmed all Sprint 202 findings unchanged: `__init__.py` lazy shim clean, `Company` and `FinancialHistory` ORM models active and unchanged, 3 production callers verified, boundary direction stable, zero provider coupling, zero stale imports, all Sprint 198/200 removals remain absent; 1681 tests passed, RC2 green, demo passes. Models cleanup track formally CLOSED Sprint 203. Sprint 204 recommended: release candidate checkpoint across 24 closed cleanup tracks. Sprint 204 RC checkpoint: all 24 closed tracks verified stable; Sprint 203 models closure confirmed (Company 10 cols, FinancialHistory 15 cols, __all__ unchanged, lazy shim clean); Sprint 198/200 removals confirmed absent (6 targets); database/services exports importable; config/database/services boundary stable; 24 packages importable; 12 deleted modules absent; 7 retired CLI commands non-callable; empty CLI groups absent; provider boundaries unchanged or intentionally classified; atlas.analysis.__all__ remains 9 exports; 1681 passed, 3 skipped; RC2 green; demo passes. Sprint 205 recommended: audit atlas/suitability/ package. Sprint 205 audited `atlas/suitability/` (2 modules, 642 lines): 7 active exports all with production callers (CLI, dashboard, comparison, watchlist_review, portfolio_review, risk_drift), no stale exports, no dead helpers, no provider coupling, no forbidden language (anti-advice disclaimer present), `check_suitability_assessment` correctly absent from active exports, boundary direction correct (no CLI/database/provider imports), no stale references from closed cleanup tracks, no overlap with decision/risk/principles/evidence; 1692 tests passed, RC2 green, demo passes. Sprint 206 recommended: close suitability cleanup track (no cleanup warranted). See `docs/SuitabilityCleanupPlan.md`. Sprint 206 confirmed all Sprint 205 findings unchanged: 7 exports active and importable, 6 production callers unchanged, `atlas suitability analyze` CLI command active, output language clean (anti-advice disclaimer present, no forbidden language), boundary direction stable, provider-free, no stale imports from closed tracks, no overlap with decision/risk/principles/evidence; 1692 tests passed, RC2 green, demo passes. Suitability cleanup track formally CLOSED Sprint 206. Sprint 207 recommended: release candidate checkpoint across 25 closed cleanup tracks. Sprint 207 RC checkpoint: all 25 closed tracks verified stable; Sprint 206 suitability closure confirmed (7 exports importable, CLI active, language clean, provider-free); Sprint 203 models closure confirmed (Company + FinancialHistory unchanged); Sprint 198/200 removals confirmed absent; database/services importable; config/database/services boundary stable; 24 packages importable; 12 deleted modules absent; 7 retired CLI commands non-callable; empty CLI groups absent; provider boundaries unchanged or intentionally classified; 1692 passed, 3 skipped; RC2 green; demo passes. Sprint 208 recommended: define Atlas v1 operating mode (productization track). Sprint 208 defined Atlas v1 operating mode: product boundary (CLI-first, deterministic, local-only, long-term investor focus), 8 candidate v1 workflows mapped against 14 active packages (all 8 included), Weekly Investment Review selected as flagship workflow, Company Review / Watchlist Review / Decision Memo specifications written, v1 input model (local, file-based, no live data required), v1 output model (10 output types, all with language guardrails), usable output criteria defined, out-of-scope list codified, 10 guardrail principles. Created `docs/AtlasV1OperatingMode.md`. No runtime files changed. 1692 passed, 3 skipped, RC2 green. Sprint 209 recommended: specify the Atlas Weekly Investment Review workflow (full specification: input data structures, workflow steps, output section definitions, CLI entrypoint design, missing implementation gaps, implementation plan, acceptance criteria). Sprint 209 specified the Weekly Investment Review workflow: 12 workflow steps, 10 output sections, CLI entrypoint `atlas weekly-review`, all input formats specified (portfolio, watchlist, investor profile, journal, company facts, financials), safe/forbidden language reference, failure behavior, implementation gap analysis (8 gap categories), acceptance criteria, example output skeleton. Key findings: existing parsers reusable for portfolio, profile, journal; WatchlistInput parser gap (accepts only tickers, not rich v1 format); InvestorProfile missing principles/constraints fields; no company_facts/ convention; no atlas weekly-review command; no multi-section renderer; no non-actions generator. No runtime files changed. 1692 passed, 3 skipped, RC2 green. Sprint 210 recommended: implement local input schemas — create examples/weekly_review/ sample files, rich watchlist parser, WeeklyReviewInput loader with validation, and guardrail tests. See docs/AtlasWeeklyInvestmentReviewSpec.md. Sprint 210 implemented local input schemas: created atlas/weekly_review/ package (top-level, 2 modules: __init__.py + inputs.py), WeeklyReviewPortfolioInput (positions[] + accounts[] formats), WeeklyReviewWatchlistInput (v1 rich item format with status/evidence_needed/open_questions/manual_observations, legacy status alias mapping), WeeklyReviewInputWarning, WeeklyReviewInputPaths, WeeklyReviewLoadResult, load_weekly_review_inputs() loader; created 7 example files under examples/weekly_review/; created 35 guardrail tests in tests/test_weekly_review_inputs_sprint210.py. No provider/network imports. No forbidden language in sample files or warnings. 1727 passed, 3 skipped, RC2 green. Sprint 211 recommended: implement atlas weekly-review command skeleton (load inputs, validate, report warnings, print placeholder structure; no full orchestration). Sprint 211 implemented the CLI skeleton: registered atlas weekly-review as root-level Typer command; created atlas/weekly_review/render.py with render_weekly_review_skeleton() (all 10 section headings, safe placeholder content, Section 10 always non-empty, evidence gaps from watchlist surfaced in Section 8, deferred items in Section 10); lazy import inside command; zero provider/network imports; 28 new tests all passing; 1755 passed, 3 skipped, RC2 green. Sprint 212 recommended: implement Weekly Investment Review renderer — replace placeholder content with deterministic output from loaded inputs (portfolio summary, per-item watchlist review, open decisions, consolidated evidence gaps, non-actions from real data). Sprint 212 implemented deterministic renderer: atlas/weekly_review/render.py rewritten with render_weekly_review(); render_weekly_review_skeleton() kept as backward-compatible alias; WeeklyReviewLoadResult extended with journal_entries field (raw dicts, zero-cost when absent); all 10 sections now produce input-derived content; Section 10 always non-empty; forbidden language clean; zero provider/network imports added; 63 new tests; 1818 passed, 3 skipped, RC2 green, demo passes. Sprint 213 recommended: Run real portfolio trial — run atlas weekly-review on a realistic portfolio/watchlist/profile bundle and identify friction before wiring deeper engines. Sprint 213 ran trial on 11-holding, 5-watchlist-item realistic bundle (examples/weekly_review_realistic/); made 6 targeted improvements: profile principles/constraints/risk_tolerance/time_horizon loaded into WeeklyReviewLoadResult, per-ticker company facts/financials presence check, _preview_scope_notes() helper strips markdown, combined top-2 concentration note, single-position threshold 30%→25%; 54 new tests; 1872 passed, 3 skipped, RC2 green, demo passes. Findings documented in docs/WeeklyReviewTrialFindings.md. Sprint 214 recommended: journal entry aging alerts.

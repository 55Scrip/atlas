...
- **Final:** Reasoning (ADR-001); Judgment (ADR-002); Knowledge (ADR-003).
...
Knowledge (Final — ADR-003) is deliberately not placed in the chain above. Its relationship to Evidence, Observation, and Reality is explicitly recorded as unresolved by ADR-003 itself (Remaining Open Questions); inserting it at a specific position in this illustrative sequence would assert a relationship ADR-003 does not claim. Knowledge is represented only in the Primitive Status Table below, via its argued dependency on Reasoning (ADR-001, ADR-003).
...
| Reasoning | Final | ADR-001 | Knowledge (per ADR-001; Knowledge's own nature is now characterized in ADR-003) | None currently open in ADR-001 | Judgment (per ADR-001: "a completed Reasoning Act produces an explicit Judgment") |
| Knowledge | Final | ADR-003 | Reasoning (per ADR-001 §1–2, as relied upon by ADR-003's Current Working Characterization, condition 1) | Relationship to Evidence, Observation, and Reality (ADR-003, unresolved); whether this dependence on Reasoning is essential or merely provisional (ADR-003, unresolved); whether Knowledge decomposes into determinate, independently identifiable portions or must be read at the level of the whole established body (ADR-003, unresolved, not settled either way); the process, if any, by which material gains or loses established standing (ADR-003, unresolved) | None asserted by ADR-003 — Knowledge is depended upon by Reasoning (ADR-001) for what it operates over; ADR-003 does not describe Knowledge as producing anything of its own |
...
- ADR-001 states that Reasoning operates over established **Knowledge**. This chain instead places **Evidence** immediately prior to Reasoning. Whether Evidence and Knowledge name the same concept, or Evidence is a distinct, not-yet-defined primitive with its own relationship to Knowledge, is unresolved. ADR-003 (Final) does not resolve this either — it explicitly leaves Knowledge's relationship to Evidence, Observation, and Reality as one of its own recorded open boundaries (see below).
...
- ADR-003 (Final) characterizes Knowledge as established material — material that stands as a candidate for Reasoning's examination and is not currently under active revision or dispute — without committing to whether that material is representational, factual, or of another kind. Three candidate categories (representational content, fact/state of affairs, the strongest available relational account) were tested and rejected; ADR-003 does not claim this exhausts every possible category. ADR-003 also records, as explicitly stated boundaries rather than blockers to its own Final status: whether Knowledge's dependence on Reasoning for its characterization is essential or merely provisional; whether Knowledge decomposes into determinate, independently identifiable portions or must instead be understood at the level of the whole established body (neither reading is adopted); and Knowledge's relationship to Evidence, Observation, and Reality, which remains exactly as unresolved as before (see above). This creates a disclosed, one-directional relationship, not a circularity: Reasoning's own ontological category is settled independently of Knowledge (ADR-001 §1–2), while Knowledge's characterization of established standing currently relies on Reasoning already being so characterized (ADR-003) — ADR-003 states this dependence explicitly without asserting whether it is essential or an artifact of this track's own sequencing.

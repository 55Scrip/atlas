...
| **Legacy module** | `atlas.analysis.watchlist` — **`WatchlistEngine` deleted Sprint 99**; file retained as types-only module |
| **Engine status** | `WatchlistEngine` deleted Sprint 99. `atlas/analysis/watchlist.py` slimmed to `Watchlist`/`WatchlistItem` only. Type-only imports in 7 production modules retained; full file deletion deferred to Sprint 100+. |

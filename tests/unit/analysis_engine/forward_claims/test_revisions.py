...
#: Reporting periods, the provider's own fiscal-quarter labels. Stage 3.1
#: orders one company's claims by these, never by a timestamp.
Q1, Q2, Q3, Q4 = "2026Q1", "2026Q2", "2026Q3", "2026Q4"
EXTRACTED = datetime(2026, 11, 20, tzinfo=timezone.utc)
...
def claim(
    *,
    ...
    return ForwardClaim(
        id=claim_id or f"{company}:{subject.value}:{horizon}:{period}",
        company=company,
...
class TestPointSemantics:
    def test_a_higher_figure_is_raised(self):
        assert compare_claims(claim(low=100), claim(low=110, period=Q2)).revision_type is RevisionType.RAISED

    def test_a_lower_figure_is_lowered(self):
        assert compare_claims(claim(low=100), claim(low=90, period=Q2)).revision_type is RevisionType.LOWERED

    def test_the_same_figure_is_reaffirmed(self):
        assert compare_claims(claim(low=100), claim(low=100, period=Q2)).revision_type is RevisionType.REAFFIRMED


class TestRangeSemantics:
    def test_both_ends_up_is_raised(self):
        old = claim(bound=ClaimBound.RANGE, low=180, high=190)
        new = claim(bound=ClaimBound.RANGE, low=195, high=205, period=Q2)
    ...
    def test_both_ends_down_is_lowered(self):
        ...
        new = claim(bound=ClaimBound.RANGE, low=180, high=190, period=Q2)
    ...
    def test_an_identical_range_is_reaffirmed(self):
        ...
        new = claim(bound=ClaimBound.RANGE, low=6.8, high=7.6, period=Q2)
    ...
    def test_ranges_may_overlap_and_still_be_raised_when_both_ends_moved_up(self):
        ...
        old = claim(bound=ClaimBound.RANGE, low=180, high=190)
        new = claim(bound=ClaimBound.RANGE, low=185, high=195, period=Q2)
        assert compare_claims(old, new).revision_type is RevisionType.RAISED
    ...
    def test_a_range_whose_ends_moved_in_opposite_directions_is_not_a_direction(self):
        ...
        new = claim(bound=ClaimBound.RANGE, low=190, high=195, period=Q2)
...
class TestBoundSemantics:
    def test_a_raised_floor_is_raised(self):
        ...
        new = claim(bound=ClaimBound.LOWER_BOUND, low=110, period=Q2)
        assert compare_claims(old, new).revision_type is RevisionType.RAISED
    ...
    def test_a_reduced_ceiling_is_lowered_numerically_and_says_nothing_more(self):
        ...
        new = claim(bound=ClaimBound.UPPER_BOUND, low=90, period=Q2)
        assert compare_claims(old, new).revision_type is RevisionType.LOWERED
...
class TestComparability:
    ...
    def test_claims_that_mean_different_things_are_never_compared(self, kwargs, expected):
        result = compare_claims(claim(), claim(period=Q2, **kwargs))
        assert not result.is_revision
...
class TestEconomicNeutrality:
    ...
    def test_raising_capex_and_raising_revenue_are_the_same_fact(self):
        ...
        revenue = compare_claims(
            ...
            claim(subject=ClaimSubject.REVENUE, low=110, period=Q2),
        )
        capex = compare_claims(
            claim(subject=ClaimSubject.CAPITAL_EXPENDITURE, low=100),
            claim(subject=ClaimSubject.CAPITAL_EXPENDITURE, low=110, period=Q2),
        )
...
class TestOrdering:
    def test_the_older_claim_must_have_been_reported_first(self):
        newer, older = claim(low=110, period=Q2), claim(low=100, period=Q1)
    ...
    def test_equal_timestamps_are_ambiguous_rather_than_broken_by_a_record_id(self):
        a = claim(low=100, period=Q1, claim_id="aaa")
        b = claim(low=110, period=Q1, claim_id="zzz")
    ...
    def test_the_horizon_never_decides_which_claim_came_first(self):
        old = claim(low=100, horizon="2027", period=Q1)
        new = claim(low=110, horizon="2027", period=Q2)
        assert compare_claims(old, new).revision_type is RevisionType.RAISED
...
class TestHistory:
    def _series(self):
        return (
            claim(low=100, period=Q1, claim_id="c1"),
            claim(low=110, period=Q2, claim_id="c2"),
            claim(low=105, period=Q3, claim_id="c3"),
            claim(low=105, period=Q4, claim_id="c4"),
        )
    ...
    def test_grouping_keys_on_everything_that_must_match(self):
        groups = group_claims(
            (claim(subject=ClaimSubject.REVENUE), claim(subject=ClaimSubject.CAPITAL_EXPENDITURE, period=Q2))
        )
        assert len(groups) == 2
...
class TestProvenanceAndVersioning:
    def test_a_revision_can_be_traced_to_both_of_its_operands(self):
        revisions, _ = detect_revisions((claim(low=100, period=Q1, claim_id="c1"),
                                         claim(low=110, period=Q2, claim_id="c2")))
        ...
        assert (evidence.prior_source_period, revision.new_source_period) == ("2026Q1", "2026Q2")
        assert revision.stated_by_title == "Chief Financial Officer"

    def test_every_revision_records_which_rules_produced_it(self):
        revisions, _ = detect_revisions((claim(low=100, period=Q1), claim(low=110, period=Q2)))
    ...
    def test_withdrawal_is_not_inferred_from_a_claim_simply_not_recurring(self):
        ...
        revisions, unrevised = detect_revisions((claim(low=100, period=Q1),))
        assert revisions == ()
        assert set(unrevised.values()) == {NonRevisionReason.NO_PRIOR_CLAIM}

...
class TestEvidenceCapture:
    ...
    def test_supports_is_accepted(self):
        evidence = Evidence.capture(
            statement=_STATEMENT, direction="SUPPORTS", observed_at=_OBSERVED_AT
        )
        assert evidence.direction == Direction.SUPPORTS

    def test_challenges_is_accepted(self):
        evidence = Evidence.capture(
            statement=_STATEMENT, direction="CHALLENGES", observed_at=_OBSERVED_AT
        )
        assert evidence.direction == Direction.CHALLENGES

    def test_rejects_invalid_direction(self):
        with pytest.raises(InvalidDirectionError):
            Evidence.capture(statement=_STATEMENT, direction="PROVES", observed_at=_OBSERVED_AT)
    ...
    def test_blank_note_normalizes_to_none(self, blank):
        evidence = Evidence.capture(
            statement=_STATEMENT, direction=Direction.SUPPORTS, observed_at=_OBSERVED_AT, note=blank
        )
        assert evidence.note is None
    ...
    def test_rejects_missing_observed_at(self):
        with pytest.raises(InvalidObservedAtError):
            Evidence.capture(statement=_STATEMENT, direction=Direction.SUPPORTS, observed_at=None)
    ...
    def test_requires_a_statement(self):
        with pytest.raises(TypeError):
            Evidence.capture(direction=Direction.SUPPORTS, observed_at=_OBSERVED_AT)

    def test_requires_a_direction(self):
        with pytest.raises(TypeError):
            Evidence.capture(statement=_STATEMENT, observed_at=_OBSERVED_AT)

...
nothing outside `atlas/alpha/canonical_security/` and this test
directory imports it. If this test ever fails, some other change wired
this foundation into the live pipeline without updating this guard --
exactly the kind of silent, undocumented integration Sprint M's own
scope forbids ("nothing in the current enrichment pipeline should begin
using it yet").
"""
...
def test_no_file_outside_this_package_and_its_own_tests_imports_it() -> None:
    ...
    assert offending == [], (
        ...
        "foundation is not yet meant to be wired into anything: " + ", ".join(offending)
    )

"""Tests for `atlas.alpha.investment_decision.service.InvestmentDecisionService`
-- built through real, unmodified Case/Decision/Observation/Portfolio/
Watchlist/Monitoring/EvidenceGraph/DecisionReadiness persistence, the
same harness pattern `tests/unit/alpha/decision_readiness/test_service.py`
already established (this package composes exactly that service, plus
Stance)."""
...
class TestSynthesizeForCase:
    def test_returns_none_for_a_case_that_does_not_exist(self, harness):
        assert harness.investment_decision_service.synthesize_for_case(str(uuid.uuid4())) is None

    def test_a_case_with_no_data_reaches_no_decision(self, harness):
        """No coverage -> Decision Readiness `UNKNOWN` -> Decision
        Support `INSUFFICIENT_EVIDENCE` -> `NO_DECISION` -- the
...
class TestCompare:
    def test_returns_none_when_either_ticker_is_unknown(self, harness):
        ...
        assert harness.investment_decision_service.compare("NVDA", "ZZZZZ") is None

    def test_compares_two_real_known_tickers(self, harness):
        harness.import_holding("NVDA")
        harness.add_to_watchlist("MSFT")
        comparison = harness.investment_decision_service.compare("NVDA", "MSFT")
        assert comparison is not None
        assert comparison.a.action.value == "no_decision"
        assert comparison.b.action.value == "no_decision"


class TestPortfolioActionDistribution:
    def test_covers_only_portfolio_holdings_not_watchlist(self, harness):
        ...
        all_tickers = {t for tickers in distribution.values() for t in tickers}
        assert all_tickers == {"NVDA"}

    def test_a_holding_with_no_data_lands_in_no_decision(self, harness):
        harness.import_holding("NVDA")
        distribution = harness.investment_decision_service.portfolio_action_distribution()

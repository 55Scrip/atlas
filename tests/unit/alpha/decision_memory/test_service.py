"""Tests for `atlas.alpha.decision_memory.service.DecisionMemoryService`
...
RecommendationConviction/DecisionPath/OpportunityCost persistence, the
same harness pattern `tests/unit/alpha/opportunity_cost/test_service.py`
already established. Also exercises the append-only discipline directly
(Deliverable 12's own central claim) -- a real change appends a new
row and preserves the old one; an unchanged re-read never does."""
...
class TestAssessForCase:
    def test_returns_none_for_a_case_that_does_not_exist(self, harness):
        assert harness.decision_memory_service.assess_for_case(str(uuid.uuid4())) is None

    def test_a_case_with_no_data_still_returns_a_real_baseline_memory(self, harness):
        case_id = harness.import_holding("NVDA")
        memory = harness.decision_memory_service.assess_for_case(case_id)
        assert memory is not None
        assert memory.current_snapshot.action.value == "no_decision"
        assert memory.previous_snapshot is None
        assert memory.latest_change is None
        assert len(memory.history.entries) == 1
        assert memory.history.entries[0].change.is_baseline is True

    def test_repeated_calls_never_append_a_duplicate_row(self, harness):
        """Deliverable 12's own central claim, exercised directly:
        re-reading unchanged state is a true no-op, never a new row."""
        case_id = harness.import_holding("NVDA")
...
class TestAppendOnlyHistory:
    def test_a_real_change_appends_a_new_row_and_preserves_the_old_one(self, harness):
        """The exact "new snapshot appended, previous snapshot
        preserved" behavior Deliverable 12 requires -- forced here by
        writing a second, structurally-different snapshot directly
        through the repository (the same one the service itself uses),
        since this harness's own upstream engines have no real lever to
        force a live transition deterministically."""
        case_id = harness.import_holding("NVDA")
        first_memory = harness.decision_memory_service.assess_for_case(case_id)
        first_hash = first_memory.current_snapshot.content_hash
        ...
        forced_inputs = DecisionSnapshotInputs(
        ...
        forced_snapshot = build_snapshot(case_id, forced_inputs, recorded_at=datetime.now(timezone.utc))
        forced_change = detect_decision_change(first_memory.current_snapshot, forced_snapshot, detected_at=forced_snapshot.recorded_at)
        written = harness.decision_memory_repository.add(case_id, forced_snapshot, forced_change, ticker="NVDA")
        assert written is True

        # `get_history` alone (not `assess_for_case`, which would
        # re-run live computation and append a *third*, real row on
        # top) is the direct way to confirm the forced row is now
        # durably recorded alongside the original -- neither one
        # overwritten.
        history = harness.decision_memory_repository.get_history(case_id)
        recorded_hashes = {entry.snapshot.content_hash for entry in history}
        assert first_hash in recorded_hashes
        assert forced_snapshot.content_hash in recorded_hashes
        assert len(history) == 2
        assert history[-1].snapshot.content_hash == forced_snapshot.content_hash
        assert history[0].snapshot.content_hash == first_hash

        latest = harness.decision_memory_repository.get_latest(case_id)
        previous = harness.decision_memory_repository.get_previous(case_id)
        assert latest.content_hash == forced_snapshot.content_hash
        assert previous.content_hash == first_hash
...
class TestCompare:
    def test_returns_none_when_either_ticker_is_unknown(self, harness):
        ...
        assert harness.decision_memory_service.compare("NVDA", "ZZZZZ") is None

    def test_compares_two_real_known_tickers(self, harness):
        harness.import_holding("NVDA")
        harness.add_to_watchlist("MSFT")
        comparison = harness.decision_memory_service.compare("NVDA", "MSFT")
...
class TestPortfolioDecisionMemoryBreakdown:
    def test_covers_only_portfolio_holdings_not_watchlist(self, harness):
        harness.import_holding("NVDA")
        harness.add_to_watchlist("MSFT")
        breakdown = harness.decision_memory_service.portfolio_decision_memory_breakdown()
        all_tickers = set(breakdown.recently_changed) | set(breakdown.stable)

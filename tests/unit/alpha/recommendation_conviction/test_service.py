"""Tests for `atlas.alpha.recommendation_conviction.service
.RecommendationConvictionService` -- built through real, unmodified
Case/Decision/Observation/Portfolio/Watchlist/Monitoring/EvidenceGraph/
DecisionReadiness/InvestmentDecision persistence, the same harness
pattern `tests/unit/alpha/investment_decision/test_service.py` already
established (this package composes exactly that service, plus
EvidenceGraph directly)."""
...
class TestAssessForCase:
    def test_returns_none_for_a_case_that_does_not_exist(self, harness):
        assert harness.recommendation_conviction_service.assess_for_case(str(uuid.uuid4())) is None

    def test_a_case_with_no_data_is_unavailable(self, harness):
        """No coverage -> Investment Decision `NO_DECISION` ->
        Conviction Strength `UNAVAILABLE` -- the natural, unforced
        consequence of the real upstream chain."""
        case_id = harness.import_holding("NVDA")
        conviction = harness.recommendation_conviction_service.assess_for_case(case_id)
        assert conviction is not None
        assert conviction.strength.value == "unavailable"
...
class TestChangeForCase:
    def test_first_computation_produces_no_change(self, harness):
        case_id = harness.import_holding("NVDA")
        assert harness.recommendation_conviction_service.change_for_case(case_id) is None
...
class TestCompare:
    def test_returns_none_when_either_ticker_is_unknown(self, harness):
        ...
        assert harness.recommendation_conviction_service.compare("NVDA", "ZZZZZ") is None

    def test_compares_two_real_known_tickers(self, harness):
        harness.import_holding("NVDA")
        harness.add_to_watchlist("MSFT")
        comparison = harness.recommendation_conviction_service.compare("NVDA", "MSFT")
        assert comparison is not None
        assert comparison.a.strength.value == "unavailable"
        assert comparison.b.strength.value == "unavailable"

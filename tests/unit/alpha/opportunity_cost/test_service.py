"""Tests for `atlas.alpha.opportunity_cost.service.OpportunityCostService`
...
RecommendationConviction/DecisionPath persistence, the same harness
pattern `tests/unit/alpha/decision_path/test_service.py` already
established (this package composes exactly that service, plus
InvestmentDecision/RecommendationConviction directly)."""
...
class TestAssessForCase:
    def test_returns_none_for_a_case_that_does_not_exist(self, harness):
        assert harness.opportunity_cost_service.assess_for_case(str(uuid.uuid4())) is None

    def test_a_case_with_no_data_still_returns_a_real_result(self, harness):
        """No coverage -> `NO_DECISION` -> no competing-Case
        alternatives are constructible (nothing has a real `BUY`/`ADD`
        action yet either), but the call itself succeeds honestly
        rather than erroring."""
        case_id = harness.import_holding("NVDA")
        opportunity_cost = harness.opportunity_cost_service.assess_for_case(case_id)
        assert opportunity_cost is not None
        assert opportunity_cost.current_action.value == "no_decision"
    ...
    def test_repeated_calls_are_idempotent_when_nothing_changed(self, harness):
        ...
        assert first.tradeoffs == second.tradeoffs


class TestChangeForCase:
    def test_first_computation_produces_no_change(self, harness):
        case_id = harness.import_holding("NVDA")
        assert harness.opportunity_cost_service.change_for_case(case_id) is None
...
class TestCompare:
    def test_returns_none_when_either_ticker_is_unknown(self, harness):
        ...
        assert harness.opportunity_cost_service.compare("NVDA", "ZZZZZ") is None

    def test_compares_two_real_known_tickers(self, harness):
        harness.import_holding("NVDA")
        harness.add_to_watchlist("MSFT")
        comparison = harness.opportunity_cost_service.compare("NVDA", "MSFT")
        assert comparison is not None
        assert comparison.conviction.a.case_id is not None
        assert comparison.path.a.case_id is not None


class TestPortfolioOpportunityCostBreakdown:
    def test_covers_only_portfolio_holdings_not_watchlist_for_the_primary_buckets(self, harness):
        harness.import_holding("NVDA")
        harness.add_to_watchlist("MSFT")
        breakdown = harness.opportunity_cost_service.portfolio_opportunity_cost_breakdown()
        primary_tickers = (
            set(breakdown.holdings_competing_for_capital)
            | set(breakdown.waiting_preferable)

"""Structural isolation tests for reflection_exploration (ATLAS-012).
...
MODULE_DIR = REPO_ROOT / "atlas" / "core" / "application" / "reflection_exploration"
...
_FORBIDDEN_MODULE_PREFIXES = (
    ...
    # ATLAS-012-D Ch5: Reflection Exploration must never construct,
    # call, or turn into a Reflection Comparison.
    "atlas.core.application.reflection_comparison",
)

# exploration.py, exceptions.py, and query.py never touch an Engine, a
...
_NO_SQLALCHEMY_AT_ALL_FILES = ("exploration.py", "exceptions.py", "query.py")
...
def test_reflection_exploration_never_imports_other_understanding_or_sibling_capabilities() -> None:
...
def test_exploration_exceptions_and_query_import_no_sqlalchemy_at_all() -> None:
    for filename in _NO_SQLALCHEMY_AT_ALL_FILES:
        ...
        assert not sqlalchemy_imports, (
            f"reflection_exploration/{filename} must not import sqlalchemy at all "

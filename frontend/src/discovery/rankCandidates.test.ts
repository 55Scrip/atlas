...
describe("rankCandidates", () => {
  it("puts excellent Fit with no conflicting Stance in Highest opportunity", () => {
    const result = rankCandidates([candidate({ fit: "excellent", stance: "maintain" })]);
  ...
  it("puts good Fit with an increasing Stance in Highest opportunity", () => {
    const result = rankCandidates([candidate({ fit: "good", stance: "increase" })]);
    expect(result.highest.map((c) => c.ticker)).toEqual(["AAA"]);
  });
  ...
  it("moves a strong Fit with a critical Stance to Worth reviewing, not Highest opportunity (a real tension, not a confident top pick)", () => {
    const result = rankCandidates([candidate({ fit: "excellent", stance: "avoid_decision" })]);
  ...
  it("moves a weak Fit with a positive Stance to Worth reviewing", () => {
    const result = rankCandidates([candidate({ fit: "weak", stance: "increase" })]);
    expect(result.worthReviewing.map((c) => c.ticker)).toEqual(["AAA"]);
  });

  it("moves a neutral-Fit candidate with an elevated Agenda priority to Worth reviewing", () => {
    const result = rankCandidates([candidate({ fit: "neutral", stance: null, priority: "high" })]);
    expect(result.worthReviewing.map((c) => c.ticker)).toEqual(["AAA"]);
  });

  it("puts a weak/poor Fit with no other signal in Everything else", () => {
    const result = rankCandidates([
      candidate({ ticker: "WEAK", fit: "weak", stance: "maintain", priority: null }),
      candidate({ ticker: "POOR", fit: "poor", stance: null, priority: "low" }),
    ]);
  ...
  it("puts an unassessed candidate (fit null) in Everything else, never fabricating a tier", () => {
    const result = rankCandidates([candidate({ ticker: "NEW", fit: null })]);
  ...
  it("never pads Highest opportunity below the honest count -- zero qualifying candidates means an empty list", () => {
    const result = rankCandidates([candidate({ fit: "poor" })]);
    expect(result.highest).toEqual([]);
  });

  it("caps Highest opportunity at 5 and moves the overflow to Worth reviewing, never dropping it", () => {
    const candidates = Array.from({ length: 7 }, (_, i) =>
      candidate({ ticker: `T${i}`, caseId: `case-${i}`, fit: "excellent", stance: "maintain" }),
    );
  ...
  it("ranks within a tier by Fit first, then Stance, then Agenda priority, then ticker", () => {
    const result = rankCandidates([
      candidate({ ticker: "GOOD_MAINTAIN", fit: "good", stance: "maintain" }),
      candidate({ ticker: "EXCELLENT_MAINTAIN", fit: "excellent", stance: "maintain" }),
      candidate({ ticker: "EXCELLENT_INCREASE", fit: "excellent", stance: "increase" }),
    ]);
  ...
  it("breaks a genuine tie alphabetically by ticker, never claiming further differentiation", () => {
    const result = rankCandidates([
      candidate({ ticker: "ZZZ", fit: "excellent", stance: "maintain" }),
      candidate({ ticker: "AAA", fit: "excellent", stance: "maintain" }),
    ]);
